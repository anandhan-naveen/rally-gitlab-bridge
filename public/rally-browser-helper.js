(async () => {
  ...
  const quarterField = scopeType === 'quarter' ? ((prompt('Rally internal quarter field name', 'c_Quarter') || 'c_Quarter').trim()) : null;
  const baseFields = ['ObjectID','FormattedID','Name','Description','ScheduleState','PlanEstimate','Owner','Project','Iteration','Release','PortfolioItem','LastUpdateDate','Tasks'];
  const fetchFields = [...baseFields, ...(quarterField ? [quarterField] : [])].join(',');
  async function query(endpoint, queryText, fields = 'true') {
    const all = []; let start = 1; const pagesize = 200;
    while (true) {
      const u = new URL(`${api}/${endpoint}`);
      u.searchParams.set('query', queryText); u.searchParams.set('fetch', fields); u.searchParams.set('start', String(start)); u.searchParams.set('pagesize', String(pagesize));
      u.searchParams.set('projectScopeDown', 'true'); u.searchParams.set('projectScopeUp', 'false');
  ...
  try {
    let stories = [];
    const safe = scopeValue.replaceAll('"', '\\"');
    if (scopeType === 'story') stories = await query('hierarchicalrequirement', `(FormattedID = "${safe}")`, fetchFields);
    else if (scopeType === 'feature') {
      const f = await query('portfolioitem/feature', `(FormattedID = "${safe}")`, 'ObjectID,FormattedID,Name,Project');
      ...
      stories = await query('hierarchicalrequirement', `(${quarterField} = "${safe}")`, fetchFields);
    }
    for (let i = 0; i < stories.length; i++) {
      const s = stories[i];
      s._tasks = await query('task', `(WorkProduct = /hierarchicalrequirement/${s.ObjectID})`, 'ObjectID,FormattedID,Name,Description,State,Estimate,Actuals,Owner,WorkProduct,LastUpdateDate');
      console.log(`Rally Bridge: ${i + 1}/${stories.length} ${s.FormattedID} (${s._tasks.length} tasks)`);
    }
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `rally-${scopeType}-${scopeValue.replace(/[^a-z0-9_-]+/gi, '_')}.json`;
    document.body.appendChild(a); a.click(); a.remove(); setTimeout(() => URL.revokeObjectURL(a.href), 5000);
    alert(`Rally snapshot created: ${stories.length} stories. Upload the downloaded JSON to http://127.0.0.1:8000.`);
  } catch (e) {
    console.error('Rally Bridge helper failed', e);
    alert(`Rally snapshot failed: ${e.message}\n\nNo Rally data was changed.`);
  }
})();
